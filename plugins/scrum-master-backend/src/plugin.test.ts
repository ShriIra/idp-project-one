...
import { scrumMasterPlugin } from './plugin';
...
describe('plugin', () => {
  it('should create and read TODO items', async () => {
    const { server } = await startTestBackend({
      features: [scrumMasterPlugin],
    });

    await request(server).get('/api/scrum-master/todos').expect(200, {
      items: [],
    });

    const createRes = await request(server)
      .post('/api/scrum-master/todos')
      .send({ title: 'My Todo' });
    ...
    await request(server)
      .get('/api/scrum-master/todos')
      .expect(200, {
        items: [createdTodoItem],
      });

    await request(server)
      .get(`/api/scrum-master/todos/${createdTodoItem.id}`)
