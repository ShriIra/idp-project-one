...
import { jiraProjectPlugin } from './plugin';
...
describe('plugin', () => {
  it('should create and read TODO items', async () => {
    const { server } = await startTestBackend({
      features: [jiraProjectPlugin],
    });

    await request(server).get('/api/jira-project/todos').expect(200, {
      items: [],
    });

    const createRes = await request(server)
      .post('/api/jira-project/todos')
      .send({ title: 'My Todo' });
    ...
    await request(server)
      .get('/api/jira-project/todos')
      .expect(200, {
        items: [createdTodoItem],
      });

    await request(server)
      .get(`/api/jira-project/todos/${createdTodoItem.id}`)
